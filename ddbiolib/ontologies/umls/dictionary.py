...
class UmlsDict(object):
    ...
    def __iter__(self):
        for sty in self.dictionary:
            for sab in self.dictionary[sty]:
                for term in self.dictionary[sty][sab]:
    ...
    def __getitem__(self,key):
        ...
        for sab in self.dictionary[key]:
            d.update(self.dictionary[key][sab])
    ...
    def get_dictionary(self, sem_types=[]):
        ...
        for sty in self.dictionary:
            if sem_types and sty not in sem_types:
                continue
            for sab in self.dictionary[sty]:
                d += self.dictionary[sty][sab].keys()
    ...
    def get_lfs(self, rvalue_default, sem_types=[], 
        ...
        for sty in self.dictionary:
            ...
            for sab in self.dictionary[sty]:
                if len(self.dictionary[sty][sab]) <= min_size:
                    continue   
                ...
                yield dict_lf_factory(self.dictionary[sty][sab], rvalue,
